...
import { ProxyFactory, GlobalAPIManager, ProxyInterceptor } from '../core/ProxyFactory';
import { generateId, headersToObject, getTimestamp } from '../utils/helpers';
...
export class FetchInterceptorPlugin extends BaseInterceptorPlugin {
  ...
  private _registerConfigCallbacks(pluginManager: PluginManager, config: HttpInterceptorConfig): void {
    const hasCallbacks =
    ...
    const fetchInterceptor: ProxyInterceptor<typeof fetch> = {
      ...
      before: async (_target, input: RequestInfo | URL, init?: RequestInit) => {
        ...
        try {
          ...
          const hookResult = await this.pluginManager.executeHook('beforeRequest', context) as RequestContext | undefined;
          if (hookResult && 'type' in hookResult && hookResult.type === RequestType.FETCH) {
            context = hookResult;
          }

          const hookResult2 = await this.pluginManager.executeHook('beforeSend', context) as RequestContext | undefined;
          ...
          (modifiedInit as any)._interceptorContext = context;
          (modifiedInit as any)._interceptorStartTime = startTime;
      ...
      after: async (_target, response: Response, _input: RequestInfo | URL, init?: RequestInit) => {
        const context = (init as any)?._interceptorContext as FetchRequestContext;
        const startTime = (init as any)?._interceptorStartTime as number;
        ...
        try {
          ...
          // Execute hooks - callbacks from config are automatically registered as plugins
          const hookResult = await this.pluginManager.executeHook('afterResponse', context, responseContext) as ResponseContext | undefined;
