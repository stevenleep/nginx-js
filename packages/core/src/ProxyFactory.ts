 ...
 * - Fixed: Memory leak prevention with cleanup methods
 */
...
export class ProxyFactory {
  ...
  // Fixed: Add cleanup method for instances
  static removeInstance(name: string): boolean {
    const instance = ProxyFactory.instances.get(name);
    if (instance) {
      instance.clear();
      ProxyFactory.instances.delete(name);
      logger.debug(`ProxyFactory instance "${name}" removed`);
      return true;
    }
    return false;
  }

  // Fixed: Add cleanup method for all instances
  static clearAllInstances(): void {
    ProxyFactory.instances.forEach((instance) => {
      instance.clear();
    });
    ProxyFactory.instances.clear();
    logger.info('All ProxyFactory instances cleared');
  }
  ...
  createFunctionProxy<F extends (...args: any[]) => any>(originalFn: F, context?: any): F {
    ...
    return async function proxiedFunction(...args: any[]) {
      ...
      for (const interceptor of self.interceptors) {
        if (interceptor.before) {
          try {
            const result = await Promise.resolve(interceptor.before(originalFn, ...modifiedArgs));
            if (result !== undefined) {
              modifiedArgs = result;
            }
          } catch (error) {
      ...
      try {
        let result = await Promise.resolve(originalFn.apply(context || this, modifiedArgs));
  ...
  createMethodProxy<T extends object, K extends keyof T>(target: T, methodName: K): T[K] {
    ...
    const ProxiedClass = class extends OriginalClass {
      constructor(...args: any[]) {
        ...
        methodsToProxy.forEach((methodName) => {
          ...
          if (typeof originalMethod === 'function') {
            (this as any)[methodName] = self.createFunctionProxy(originalMethod.bind(this), this);
          }
        });
      }
    };
...
export class GlobalAPIManager {
  ...
  static restore(apiName: string, getGlobal: () => any = () => window): boolean {
    const global = getGlobal();
  ...
  static clear(): void {
    GlobalAPIManager.originalAPIs.clear();
    logger.info('GlobalAPIManager cleared');
  }
}
