...
export class InterceptorManager {
  ...
  constructor(config: InterceptorConfig = {}) {
    ...
    this.pluginManager = new PluginManager(
      ...
      this.config.errorStrategy || 'continue'
    );
   ...
   * Improved: Sets requestTypes to support all request types by default
   */
  private _registerConfigCallbacks(): void {
    ...
    if (hasCallbacks) {
      const callbackPlugin: Plugin = {
        ...
        enabled: true,
        // Support all request types by default
        requestTypes: [RequestType.FETCH, RequestType.XHR, RequestType.WEBSOCKET],
  ...
  updateConfig(config: Partial<InterceptorConfig>): void {
    ...
    if (config.errorStrategy) {
      this.pluginManager.setErrorStrategy(config.errorStrategy);
    }
  }
  ...
  start(): void {
    ...
    if (this.config.debug) {
      console.log(
        '[InterceptorManager] Started with interceptors:',
