...
import { generateId, getTimestamp } from '@nginx-js/utils';
...
export class WebSocketInterceptorPlugin extends BaseInterceptorPlugin {
  ...
  private _registerConfigCallbacks(
    ...
    config: WebSocketInterceptorConfig
  ): void {
    const hasCallbacks =
    ...
    if (hasCallbacks) {
      const callbackPlugin: Plugin = {
        ...
        requestTypes: [RequestType.WEBSOCKET],
        ...
        void (async () => {
          try {
            const modifiedContext = (await self.pluginManager.executeHook(
              'beforeWebSocketOpen',
              context
            )) as WebSocketRequestContext | undefined;
            if (
              modifiedContext &&
              'type' in modifiedContext &&
              modifiedContext.type === RequestType.WEBSOCKET
            ) {
